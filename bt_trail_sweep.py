...
def run_cb(df, n_bars, threshold, atr_min, atr_max, dedup_b,
           am_sl, am_trail, am_mult, am_hold,
           pm_sl, pm_trail, pm_mult, pm_hold, be_atr_min=2.5):
    df2 = detect_comp(df.copy(), n_bars=n_bars, threshold=threshold)
    filt = (df2['atr'] <= atr_max) & (df2['atr'] >= atr_min) & (df2['rsi14']<70).fillna(True)
    mAM = dedup(df2['comp']&(df2['mins']>=9*60+15)&(df2['mins']<=10*60+45)&filt, dedup_b)
    mPM = dedup(df2['comp']&(df2['mins']>=13*60+15)&(df2['mins']<=14*60+15)&filt, dedup_b)
    return sim_all(df2, mAM, mPM,
                   am_sl, am_trail, am_mult, am_hold,
                   pm_sl, pm_trail, pm_mult, pm_hold, be_atr_min=be_atr_min)
...
for tf_name, df_tf, n_d, nb, atr_lo, atr_hi, ded, hold_am, hold_pm, be_min in [
    ('5m', df5, n5, 3, 2.5, 4.5, 5, 24, 12, 3.5),
    ('3m', df3, n3, 5, 1.5, 3.0, 8, 40, 20, 2.5),
    ('1m', df1, n1, 10, 0.7, 1.5, 15, 90, 45, 1.0),
]:
    print(f'\n{"="*90}')
    ...
    for ta_val in trail_vals:
        label = f'{tf_name} CB {nb}bar | Trail@+{ta_val:.0f}pts (AM=2x PM=1.5x) | BE@4 ATR>={be_min}'
        tdf = run_cb(df_tf, nb, 0.7, atr_lo, atr_hi, ded,
...
# ===================================================================
best_configs = {
    '5m': (df5, n5, 3, 2.5, 4.5, 5, 1.2, 4.0, 2.0, 24, 1.0, 3.0, 1.5, 12, 3.5),
    '3m': (df3, n3, 5, 1.5, 3.0, 8, 1.2, 4.0, 2.0, 40, 1.0, 3.0, 1.5, 20, 2.5),
    '1m': (df1, n1, 10, 0.7, 1.5, 15, 1.2, 4.0, 2.0, 90, 1.0, 3.0, 1.5, 45, 1.0),
}

for tf_name, vals in best_configs.items():
    df_tf, n_d, nb, atr_lo, atr_hi, ded, am_sl, am_trail, am_mult, am_hold, pm_sl, pm_trail, pm_mult, pm_hold, be_min = vals
    tdf = run_cb(df_tf, nb, 0.7, atr_lo, atr_hi, ded,
                 am_sl, am_trail, am_mult, am_hold,
                 pm_sl, pm_trail, pm_mult, pm_hold, be_atr_min=be_min)
